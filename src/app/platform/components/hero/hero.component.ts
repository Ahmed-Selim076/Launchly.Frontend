...
@Component({
  ...
  template: `
    <section class="relative overflow-hidden pt-20 pb-28 px-6" id="top">
      ...
      <div class="animate-reveal-in stagger-6 mt-20 max-w-4xl mx-auto relative h-[280px] sm:h-[320px]">
        @for (card of previewCards; track card.label; let i = $index) {
          <a
            appMagneticHover [magnetStrength]="8" [baseTransform]="card.transform"
            routerLink="/showcase"
    ...
    <style>
      ...
      .preview-card:hover {
        /* Shadow + stacking only — never transform. The directive above
           owns transform exclusively (base fan-out position + magnetic
           wobble); a hover rule that also touches transform would just
           re-create the original bug of the two fighting over it. */
...
export class HeroComponent {
  readonly previewCards = [
    ...
    ...card,
    transform: `translate(-50%, -50%) translateX(${card.x}px) rotate(${card.rotate}deg)`,
  }));
}
